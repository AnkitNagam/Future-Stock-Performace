...
if feat_select==1:
    ...
    if fs_type==3:
        if binning==1:                                                              ######Only work if the Target is binned###########
            ...
            sel=SelectKBest(k=2)
            fit_mod=sel.fit(data_np.astype(np.float), target_np.astype(np.float))                                         #will throw error if any negative values in features, so turn off feature normalization, or switch to mutual_info_classif
